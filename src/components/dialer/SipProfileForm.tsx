...
const buildInitialState = (values?: Partial<SipProfile>): Omit<SipProfile, "id"> => {
  ...
  if (base.provider === "telnyx") {
    const trimmedDomain = base.domain?.trim() || TELNYX_DEFAULT_DOMAIN;
    return {
      ...base,
...
export const SipProfileForm = ({
  ...
  const sanitizedState = useMemo(() => {
    ...
    next.domain = next.domain.trim() || TELNYX_DEFAULT_DOMAIN;
    next.label = next.label.trim();
    next.username = next.username.trim();
    if (next.provider === "telnyx") {
      const telnyxDomain = next.domain || TELNYX_DEFAULT_DOMAIN;
      next.transport = "wss";
    ...
    <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <select
            ...
            onChange={(event) => {
              ...
              setFormState((prev) => {
                ...
                if (value === "telnyx") {
                  const trimmedDomain = prev.domain?.trim() || TELNYX_DEFAULT_DOMAIN;
                  const previousDomain = prev.domain?.trim() || TELNYX_DEFAULT_DOMAIN;
      ...
      {formState.provider === "telnyx" ? (
        <div className="rounded-md border border-dashed border-border bg-muted/40 p-3 text-sm text-muted-foreground">
          Telnyx defaults automatically configure secure WebSocket ({getTelnyxWebsocketUrl(
            formState.domain?.trim() || TELNYX_DEFAULT_DOMAIN,
          )}), registrar ({formState.domain?.trim() || TELNYX_DEFAULT_DOMAIN}), and WSS transport. Enable advanced overrides
